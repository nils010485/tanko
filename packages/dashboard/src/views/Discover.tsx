...
import {
    IconAlert, IconCheck, IconChevronDown, IconDownload, IconEye, IconEyeOff,
    IconPlus, IconRefresh, IconSearch, IconStar, IconX
...
export default function Discover({ onAddedToLibrary }: { onAddedToLibrary: () => void }) {
    ...
    useEffect(() => {
        refreshSources().then(list => {
            ...
            const preferred = visible.find(source => source.id === 'toonily')
    ...
    const visibleSources = useMemo(() => {
        ...
        const filtered = needle
            ? base.filter(source => source.label.toLowerCase().includes(needle) || source.id.includes(needle))
            : base;
        return [...filtered]
        ...
        <div className="space-y-6">
            ...
            <Card className="p-4">
                ...
                <div className="relative" ref={comboRef}>
                    <button
                        onClick={() => { setComboOpen(open => !open); setSourceQuery(''); }}
                        ...
                        <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900 shadow-xl shadow-black/40">
                            ...
                            <div className="max-h-80 overflow-y-auto">
                                {visibleSources.map(source => (
                                    <button
                                        key={source.id}
                                        onClick={() => { setSourceId(source.id); setComboOpen(false); }}
                            ...
                            <div className="flex items-center justify-between border-t border-zinc-800 px-3 py-2 text-xs text-zinc-500">
                                <span>{showHidden ? t('discover.sourcesCount', { n: sources.length }) : t('discover.sourcesVisible', { n: sources.length - hiddenCount })}{hiddenCount > 0 && ` · ${t('discover.hiddenCount', { n: hiddenCount })}`}</span>
                                <button onClick={() => setShowHidden(value => !value)} className="flex items-center gap-1.5 text-zinc-400 transition-colors hover:text-zinc-200">
                                    {showHidden ? <><IconEyeOff size={13} /> {t('discover.hideHidden')}</> : <><IconEye size={13} /> {t('discover.showHidden')}</>}
                                </button>
                            </div>
                        </div>
                    )}
                ...
                {currentSource && (
                    <div className="mt-3 flex flex-wrap items-center gap-1.5">
                        ...
                        {currentSource.health === 'ok' && <Badge tone="green">{t('discover.statusOk')}{currentSource.healthLatencyMs ? ` · ${currentSource.healthLatencyMs} ms` : ''}</Badge>}
                        ...
                        {currentSource.tags?.slice(0, 4).map((tag: string) => <Badge key={tag}>{tag}</Badge>)}
                    </div>
                )}
            </Card>
            ...
            {searchError && (
                <Card className="flex items-start gap-3 border-red-500/40 bg-red-500/5 p-3 text-sm text-red-300">
                    ...
                    <button onClick={() => setSearchError('')} className="ml-auto text-red-400/70 hover:text-red-300"><IconX size={14} /></button>
                </Card>
            )}
                        ...
                        return (
                            <Card key={key} className="flex gap-3 p-3">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="truncate text-sm font-medium" title={manga.title}>{manga.title}</div>
                                    <div className="mt-0.5 truncate text-xs text-zinc-500">{currentSource?.label}</div>
                                    <div className="mt-2 flex flex-wrap gap-1.5">
                                        <Button small variant="ghost" onClick={() => openChapters(manga)}>{t('discover.chapters')}</Button>
            ...
            {selected && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setSelected(null)}>
                    <div className="flex max-h-[80vh] w-full max-w-lg flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl shadow-black/60" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3">
                            ...
                            <button className="flex-none rounded-md p-1 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-200" onClick={() => setSelected(null)} title={t('common.close')}>
                                <IconX size={16} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 space-y-1 overflow-y-auto p-3 text-sm">
                            {!chapters
                                ? <div className="flex items-center justify-center gap-2 py-8 text-zinc-500"><Spinner /> {t('common.loading')}</div>
                                : chapters.length === 0
                                    ? (
                                        <div className="py-8 text-center text-sm text-zinc-500">
                                            {chaptersError
                                                ? <span className="text-red-400">{chaptersError}</span>
                                                : t('discover.noChapter')}
                                        </div>
                                    )
                                        ...
                                        <div key={chapter.id} className="flex items-center justify-between gap-2 rounded-md bg-zinc-900/60 px-3 py-1.5">
                                            ...
                                            <div className="flex flex-none items-center gap-1">
                                                ...
                                                <Button
                                                    small
                                                    variant="ghost"
                                                    onClick={() => api.enqueue({ sourceId, mangaId: selected.id, mangaTitle: selected.title, chapters: [{ id: chapter.id, title: chapter.title }] })}
                                                >
                                                    <span className="flex items-center gap-1"><IconDownload size={12} /> DL</span>
                                                </Button>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="border-t border-zinc-800 p-3">
                            <Button small onClick={() => addToLibrary(selected)} disabled={addedIds.has(`${sourceId}:${selected.id}`)} loading={adding && !addedIds.has(`${sourceId}:${selected.id}`)}>
            ...
            {(previewLoading || preview !== null) && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" onClick={() => setPreview(null)}>
                    <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3">
                            ...
                            <button className="flex-none rounded-md p-1 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200" onClick={() => setPreview(null)} title={t('common.close')}>
                                <IconX size={16} />
                            </button>
                        </div>
                        <div className="flex-1 space-y-2 overflow-y-auto bg-zinc-900/40 p-3">
                            {previewLoading
                                ? <div className="flex items-center justify-center gap-2 py-10 text-zinc-500"><Spinner /> {t('discover.loadingPages')}</div>
                                : previewError
