...
function statusBadge(status: string, t: TFunction) {
    switch (status) {
        case 'downloading': return <Badge tone="blue">{t('downloads.statusDownloading')}</Badge>;
        case 'queued': return <Badge tone="zinc">{t('downloads.statusQueued')}</Badge>;
        case 'completed': return <Badge tone="green">{t('downloads.statusCompleted')}</Badge>;
        case 'failed': return <Badge tone="red">{t('downloads.statusFailed')}</Badge>;
        case 'cancelled': return <Badge tone="zinc">{t('downloads.statusCancelled')}</Badge>;
        default: return <Badge tone="zinc">{status}</Badge>;
    }
}
...
export default function Downloads({ library }: { library: LibraryEntryDto[] }) {
    ...
    useEffect(() => {
        const loadStatus = () => api.downloadStatus().then(setQueueStatus).catch(() => undefined);
        loadStatus();
    ...
    return (
        <div className="space-y-6">
            <SectionTitle
                right={
                    <div className="flex items-center gap-2">
                        {queueStatus?.paused
                            ? <Button small onClick={() => api.resumeQueue().then(load)}><IconPlay size={13} /> {t('downloads.resume')}</Button>
                            : <Button small variant="ghost" onClick={() => api.pauseQueue().then(load)}><IconPause size={13} /> {t('downloads.pauseAll')}</Button>}
                    </div>
                }
            >
                ...
                <div className="flex flex-wrap items-center gap-1">
                    {STATUS_FILTERS.map(filter => {
                        ...
                        return (
                            ...
                            >
                                {t(filter.key)}{count !== undefined && count > 0 ? ` (${count})` : ''}
                            </button>
                        );
                    })}
                </div>
            ...
            {loaded && jobs.length === 0 && (
                <EmptyState title={t('downloads.empty')} hint={t('downloads.emptyHint')} icon={<IconDownload size={28} />} />
            )}
                ...
                {jobs.map(job => (
                    <Card key={job.id} className="p-3">
                        <div className="flex items-start gap-3">
                            <Cover title={job.mangaTitle} thumbnail={job.entryId != null ? covers.get(job.entryId)?.thumbnail : undefined} coverUrl={job.entryId != null ? covers.get(job.entryId)?.coverUrl : undefined} className="h-11 w-8 flex-none rounded" />
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-zinc-500">
                                    ...
                                    {job.pagesTotal > 0 && (<><span>{t('downloads.pagesCount', { done: job.pagesDone, total: job.pagesTotal })}</span><span>·</span></>)}
                                ...
                                <div className="mt-2 flex items-center gap-2">
                                    <div className="min-w-0 flex-1"><ProgressBar value={job.progress} /></div>
                                    <span className="w-10 flex-none text-right text-xs text-zinc-400">{job.progress}%</span>
                                </div>
                            </div>
                            {(job.status === 'queued' || job.status === 'downloading') && (
                                <Button small variant="danger" onClick={() => cancel(job)} title={t('downloads.cancel')}><IconX size={13} /></Button>
                            )}
                        </div>
            ...
            {pageCount > 1 && (
                <div className="flex items-center justify-center gap-3 text-sm text-zinc-400">
                    <Button small variant="ghost" disabled={current === 0} onClick={() => setPage(current - 1)}>{t('downloads.previous')}</Button>
                    <span>{t('downloads.pageSummary', { a: current + 1, b: pageCount, n: total })}</span>
                    <Button small variant="ghost" disabled={current >= pageCount - 1} onClick={() => setPage(current + 1)}>{t('downloads.next')}</Button>
                </div>
            )}
        </div>
    );
